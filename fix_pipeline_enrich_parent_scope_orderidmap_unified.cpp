...
static std::vector<ExtractorSpec> buildExtractorSpecs() {
    ...
    // order_id_by_algo-derived text fields: use strtID:order_id_by_algo line
    ...
    // order_id_by_algo-derived FIX computed field: use FIX line where 11=order_id_by_algo
...
struct DiffRow {
    ...
    std::string orderIdByAlgoKey; // internal lookup key loaded from external map
};
...
static void collectCandidatesByOrderIdByAlgo(const std::string& orderIdByAlgo,
                                      const std::unordered_map<std::string, std::vector<int> >& byOrderIdByAlgo,
                                      std::unordered_set<int>& out) {
    if (orderIdByAlgo.empty()) return;
    std::unordered_map<std::string, std::vector<int> >::const_iterator it = byOrderIdByAlgo.find(orderIdByAlgo);
    if (it != byOrderIdByAlgo.end()) {
        for (size_t i = 0; i < it->second.size(); ++i) out.insert(it->second[i]);
    }
}

static void applyOrderIdByAlgoScopedCandidates(const std::unordered_set<int>& candidateIds,
                                        std::vector<TargetState>& targets,
                                        const std::unordered_map<std::string, std::string>& textKv,
                                        const std::unordered_map<std::string, std::string>& fixTags,
                                        const std::vector<ExtractorSpec>& specs,
...
static void buildLocalIndexes(const std::vector<int>& idxs,
                              ...
                              std::unordered_map<std::string, std::vector<int> >& byClOrd,
                              std::unordered_map<std::string, std::vector<int> >& byExec,
                              std::unordered_map<std::string, std::vector<int> >& byOrderIdByAlgo) {
    byClOrd.clear();
    byExec.clear();
    byOrderIdByAlgo.clear();
    ...
    for (size_t i = 0; i < idxs.size(); ++i) {
        ...
        if (!t.row.orderIdByAlgoKey.empty()) byOrderIdByAlgo[t.row.orderIdByAlgoKey].push_back(idx);
    }
}
...
static size_t scanSingleFileForTargets(const std::string& filePath,
    ...
    std::unordered_map<std::string, std::vector<int> > localByOrderIdByAlgo;
    buildLocalIndexes(targetIdxs, targets, localByClOrd, localByExec, localByOrderIdByAlgo);
    ...
    while (remaining > 0 && std::getline(in, line)) {
        ...
        if (!textClOrd.empty()) {
            std::unordered_set<int> orderIdByAlgoTextCandidates;
            collectCandidatesByOrderIdByAlgo(textClOrd, localByOrderIdByAlgo, orderIdByAlgoTextCandidates);
            if (!orderIdByAlgoTextCandidates.empty()) {
                applyOrderIdByAlgoScopedCandidates(orderIdByAlgoTextCandidates, targets, textKv, emptyFix,
        ...
        if (line.find("8=FIX") != std::string::npos || line.find("11=") != std::string::npos) {
            ...
            for (size_t mi = 0; mi < msgs.size() && remaining > 0; ++mi) {
                ...
                if (!fixClOrd.empty()) {
                    std::unordered_set<int> orderIdByAlgoFixCandidates;
                    collectCandidatesByOrderIdByAlgo(fixClOrd, localByOrderIdByAlgo, orderIdByAlgoFixCandidates);
                    if (!orderIdByAlgoFixCandidates.empty()) {
                        applyOrderIdByAlgoScopedCandidates(orderIdByAlgoFixCandidates, targets, textKv, fixTags,
...
static bool runEnrichDiff(const std::string& diffCsv,
    ...
    for (size_t i = 0; i < targets.size(); ++i) {
        ...
        if (itp != clOrdToOrderIdByAlgo.end()) {
            targets[i].row.orderIdByAlgoKey = itp->second; // internal order_id_by_algo-scoped lookup key
