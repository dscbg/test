...
def build_region_distribution_summary(df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    metrics = ["clearing_time_hat_min", "avg_spread_bps", "tick_day_moves", "hit_cnt", "ct_bucket_ratio"]
...
def make_boxplot(df: pd.DataFrame, metric: str, out_path: Path, title: str, clip_q: float = 0.99):
    ...
    if np.isfinite(upper):
        tmp = tmp[tmp[metric] <= upper]

    regions = sorted(list(tmp["region"].dropna().astype(str).unique()))
    labels = []
    data = []
    for r in regions:
        vals = tmp.loc[tmp["region"] == r, metric].values
        if len(vals) > 0:
            labels.append(r)
            data.append(vals)
...
def make_top_ticker_timeseries(df: pd.DataFrame, top_tickers: pd.DataFrame, plots_dir: Path):
    ...
    for region, rg in daily.groupby("region"):
        syms = top_pairs.loc[top_pairs["region"] == region, "sym"].astype(str).tolist()
        if not syms:
            continue
        ...
        for metric, short_name in metric_map.items():
            ...
            plotted = False
            for sym in syms:
                sg = rg.loc[rg["sym"].astype(str) == sym].sort_values("date")
                if sg.empty:
                    continue
                plt.plot(sg["date"], sg[metric], label=sym)
                plotted = True
            if not plotted:
                plt.close()
                continue
...
def make_region_summary_barplots(region_dist: pd.DataFrame, plots_dir: Path):
    df = region_dist.copy()
    df["region"] = df["region"].astype(str)

    metrics = [
        ("clearing_time_hat_min_median", "region_median_ct_bar.png", "Region median clearing time"),
        ("avg_spread_bps_median", "region_median_spread_bar.png", "Region median spread bps"),
        ("tick_day_moves_median", "region_median_tick_bar.png", "Region median daily tick moves"),
        ("hit_cnt_median", "region_median_hitcnt_bar.png", "Region median hit count"),
    ]
    ...
    gate_cols = ["gate_rate_hit1", "gate_rate_hit2", "gate_rate_hit3", "gate_rate_hit5"]
    gate_cols = [c for c in gate_cols if c in df.columns]
    if gate_cols:
        x = np.arange(len(df))
        width = 0.18
        plt.figure(figsize=(10, 6))
        for i, c in enumerate(gate_cols):
            plt.bar(x + (i - (len(gate_cols)-1)/2)*width, df[c], width=width, label=c)
        plt.xticks(x, df["region"])
        plt.xlabel("Region")
        plt.ylabel("Rate")
        plt.title("Gate rates by region")
        plt.legend()
        plt.tight_layout()
        out_path = plots_dir / "region_gate_rates_bar.png"
        plt.savefig(out_path, dpi=160)
        plt.close()
        print(f"[OK] wrote plot {out_path}")


def make_threshold_candidate_plots(threshold_df: pd.DataFrame, plots_dir: Path):
    df = threshold_df.copy()
    df["region"] = df["region"].astype(str)

    groups = [
        (["X_min_q20_ct", "X_min_q25_ct", "X_min_q30_ct"], "threshold_candidates_Xmin_by_region.png", "X_min candidates by region"),
        (["Y_bps_q20_spread", "Y_bps_q25_spread", "Y_bps_q30_spread"], "threshold_candidates_Ybps_by_region.png", "Y_bps candidates by region"),
        (["Z_ticks_q70_tick", "Z_ticks_q75_tick", "Z_ticks_q80_tick"], "threshold_candidates_Zticks_by_region.png", "Z_ticks candidates by region"),
    ]

    for cols, fname, title in groups:
        ...
        for i, c in enumerate(cols):
            plt.bar(x + (i - (len(cols)-1)/2)*width, df[c], width=width, label=c)
        plt.xticks(x, df["region"])
        plt.xlabel("Region")
        plt.ylabel("Value")
        plt.title(title)
    ...
    if "suggest_min_hit_cnt" in df.columns:
        plt.figure(figsize=(9, 5))
        plt.bar(df["region"], df["suggest_min_hit_cnt"])
        plt.title("Suggested min_hit_cnt by region")
        plt.xlabel("Region")
        plt.ylabel("suggest_min_hit_cnt")
        plt.tight_layout()
        out_path = plots_dir / "threshold_candidates_min_hit_cnt_by_region.png"
        plt.savefig(out_path, dpi=160)
        plt.close()
        print(f"[OK] wrote plot {out_path}")


def make_symbol_stability_plots(symbol_df: pd.DataFrame, plots_dir: Path, top_n: int = 20):
    df = symbol_df.copy()
    df["region"] = df["region"].astype(str)
    ...
    for region, g in df.groupby("region"):
        ...
        # scatter: avg_hit_cnt vs gate_hit2_rate
        if {"avg_hit_cnt", "gate_hit2_rate", "n_days"}.issubset(gg.columns):
            plt.figure(figsize=(9, 6))
            sizes = 20 + 8 * gg["n_days"].fillna(0).clip(lower=0)
            plt.scatter(gg["avg_hit_cnt"], gg["gate_hit2_rate"], s=sizes)
            plt.title(f"avg_hit_cnt vs gate_hit2_rate - {region}")
            plt.xlabel("avg_hit_cnt")
            plt.ylabel("gate_hit2_rate")
            plt.tight_layout()
            out_path = plots_dir / f"symbol_scatter_hitcnt_vs_gate_rate_{region}.png"
            plt.savefig(out_path, dpi=160)
            plt.close()
            print(f"[OK] wrote plot {out_path}")

        # scatter: median_ct vs ct_cv_by_day
        if {"median_ct", "ct_cv_by_day"}.issubset(gg.columns):
            tmp = gg.dropna(subset=["median_ct", "ct_cv_by_day"])
            if not tmp.empty:
                plt.figure(figsize=(9, 6))
                plt.scatter(tmp["median_ct"], tmp["ct_cv_by_day"])
                plt.title(f"median_ct vs ct_cv_by_day - {region}")
                plt.xlabel("median_ct")
                plt.ylabel("ct_cv_by_day")
                plt.tight_layout()
                out_path = plots_dir / f"symbol_scatter_median_ct_vs_ct_cv_{region}.png"
                plt.savefig(out_path, dpi=160)
                plt.close()
                print(f"[OK] wrote plot {out_path}")

        # top-N by gate_hit2_rate
        if "gate_hit2_rate" in gg.columns:
            top = gg.sort_values(["gate_hit2_rate", "avg_hit_cnt"], ascending=[False, False]).head(top_n)
            if not top.empty:
                plt.figure(figsize=(12, 6))
                plt.bar(top["sym"], top["gate_hit2_rate"])
                plt.title(f"Top {top_n} gate_hit2_rate tickers - {region}")
                plt.xlabel("Ticker")
                plt.ylabel("gate_hit2_rate")
                plt.xticks(rotation=75)
                plt.tight_layout()
                out_path = plots_dir / f"symbol_top{top_n}_gate_hit2_rate_{region}.png"
                plt.savefig(out_path, dpi=160)
                plt.close()
                print(f"[OK] wrote plot {out_path}")


def make_top_ticker_rank_plots(top_df: pd.DataFrame, plots_dir: Path):
    df = top_df.copy()
    df["region"] = df["region"].astype(str)
    df["sym"] = df["sym"].astype(str)

    for region, g in df.groupby("region"):
        gg = g.sort_values("rank_in_region")
        if "research_score" in gg.columns:
            plt.figure(figsize=(10, 5))
            plt.bar(gg["sym"], gg["research_score"])
            plt.title(f"Top ticker research score - {region}")
            plt.xlabel("Ticker")
            plt.ylabel("research_score")
            plt.xticks(rotation=75)
            plt.tight_layout()
            out_path = plots_dir / f"top_tickers_research_score_{region}.png"
            plt.savefig(out_path, dpi=160)
            plt.close()
            print(f"[OK] wrote plot {out_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Distribution-first analysis with layered plots for bucket_metrics outputs.")
    parser.add_argument("--root", default="out", help="Root output directory containing YYYY-MM-DD subfolders")
    parser.add_argument("--analysis_out", default="analysis_out", help="Directory to write analysis CSVs and plots")
    ...
    parser.add_argument("--plots_only", action="store_true", default=False, help="Reuse existing CSVs in analysis_out and only regenerate plots")
    ...
    plot_dirs = {
        "all": analysis_out / "plots" / "from_all_metrics",
        "region_dist": analysis_out / "plots" / "from_region_distribution",
        "region_day": analysis_out / "plots" / "from_region_day",
        "symbol": analysis_out / "plots" / "from_symbol_stability",
        "top": analysis_out / "plots" / "from_top_tickers",
        "threshold": analysis_out / "plots" / "from_threshold_candidates",
    }
    for p in plot_dirs.values():
        p.mkdir(parents=True, exist_ok=True)

    if args.plots_only:
        print(f"[INFO] plots_only mode from {analysis_out}")
        all_df = pd.read_csv(analysis_out / "all_bucket_metrics.csv")
        all_df = ensure_tick_col(all_df)

        region_dist = pd.read_csv(analysis_out / "region_distribution_summary.csv")
        region_day = pd.read_csv(analysis_out / "region_day_summary.csv")
        symbol_summary = pd.read_csv(analysis_out / "symbol_stability_summary.csv")
        top_tickers = pd.read_csv(analysis_out / "top_tickers_by_region.csv")
        threshold_candidates = pd.read_csv(analysis_out / "region_threshold_candidates.csv")
    else:
        root = Path(args.root)
        ...
        manifest = pd.DataFrame([
            ...
            {"file": "plots/", "description": "Layered PNG plots from all/summaries"},
        ])
        manifest_path = analysis_out / "analysis_manifest.csv"
        manifest.to_csv(manifest_path, index=False)
        print(f"[OK] wrote {manifest_path}")

    # plots from all_bucket_metrics
    make_boxplot(all_df, "clearing_time_hat_min", plot_dirs["all"] / "region_boxplot_clearing_time_hat_min.png",
                 "Clearing time by region")
    make_boxplot(all_df, "avg_spread_bps", plot_dirs["all"] / "region_boxplot_avg_spread_bps.png",
                 "Spread bps by region")
    make_boxplot(all_df, "tick_day_moves", plot_dirs["all"] / "region_boxplot_tick_day_moves.png",
                 "Daily tick moves by region")
    make_boxplot(all_df, "hit_cnt", plot_dirs["all"] / "region_boxplot_hit_cnt.png",
                 "Hit count by region")
    if "ct_bucket_ratio" in all_df.columns:
        make_boxplot(all_df, "ct_bucket_ratio", plot_dirs["all"] / "region_boxplot_ct_bucket_ratio.png",
                     "Clearing-time / bucket ratio by region")

    for metric in ["clearing_time_hat_min", "avg_spread_bps", "tick_day_moves", "hit_cnt"]:
        if metric in all_df.columns:
            make_hist_by_region(all_df, metric, plot_dirs["all"])
    if "ct_bucket_ratio" in all_df.columns:
        make_hist_by_region(all_df, "ct_bucket_ratio", plot_dirs["all"])

    # plots from region_distribution_summary
    make_region_summary_barplots(region_dist, plot_dirs["region_dist"])

    # plots from region_day_summary
    make_region_day_lineplots(region_day, plot_dirs["region_day"])

    # plots from symbol_stability_summary
    make_symbol_stability_plots(symbol_summary, plot_dirs["symbol"], top_n=max(args.top_n, 10))

    # plots from top_tickers_by_region
    make_top_ticker_rank_plots(top_tickers, plot_dirs["top"])
    make_top_ticker_timeseries(all_df, top_tickers, plot_dirs["top"])

    # plots from region_threshold_candidates
    make_threshold_candidate_plots(threshold_candidates, plot_dirs["threshold"])
