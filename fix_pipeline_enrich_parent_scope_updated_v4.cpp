...
static void appendParentSecondIdDebugRow(const std::string& debugCsvPath,
                                         const std::string& filePath,
                                         const std::string& firstId,
                                         const std::string& secondId,
                                         size_t matchedCandidates,
                                         const std::string& line) {
    std::ofstream out(debugCsvPath.c_str(), std::ios::app);
    if (!out.is_open()) return;

    bool writeHeader = false;
    {
        std::ifstream chk(debugCsvPath.c_str());
        writeHeader = (!chk.is_open() || chk.peek() == std::ifstream::traits_type::eof());
    }
    if (writeHeader) {
        out << "File,first_id,second_id,matched_candidate_count,LineExcerpt\n";
    }

    out << csvEscape(filePath) << ","
        << csvEscape(firstId) << ","
        << csvEscape(secondId) << ","
        << matchedCandidates << ","
        << csvEscape(lineExcerpt(line)) << "\n";
}
...
static size_t scanSingleFileForTargets(const std::string& filePath,
    ...
    const std::string parentSecondIdDebugPath = "search_results/parent_second_id_debug.csv";
    ...
    while (std::getline(in, line)) {
        ...
        std::string firstTextId = hasStrt0 ? extractNthTextValueColon(line, "id", 1) : "";
        ...
        if (hasStrt0) {
            ...
            appendParentSecondIdDebugRow(parentSecondIdDebugPath, filePath, firstTextId, textRawParentId, parentTextCandidates.size(), line);
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        while (std::getline(in, line)) {
            ...
            if (hasStrt0) {
                ...
                appendParentSecondIdDebugRow(parentSecondIdDebugPath, fullPath, firstTextId, textRawId, parentTextCandidates.size(), line);
                if (!parentTextCandidates.empty()) {
                    for (std::unordered_set<int>::const_iterator pit = parentTextCandidates.begin();
                         pit != parentTextCandidates.end(); ++pit) {
                        int idx = *pit;
...
static bool runEnrichDiff(const std::string& diffCsv,
    ...
    {
        std::ofstream trunc1("search_results/parent_second_id_debug.csv");
        std::ofstream trunc2("search_results/parent_missing_debug.csv");
    }
