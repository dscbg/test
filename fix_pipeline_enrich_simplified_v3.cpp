...
// =========================
// Output shaping for original diff CSV columns
// =========================
// If you want to drop some columns from the *original* diff CSV in the enriched output,
// add their exact header names here.
static const std::unordered_set<std::string> kDropOriginalColumns = {
    // Example:
    // "File",
};

// If you want to rename some *original* diff CSV columns in the enriched output,
// add mappings here: old_name -> new_name.
...
static std::vector<ExtractorSpec> buildExtractorSpecs() {
    ...
    // TEXT-only fields
    specs.push_back(Text("next_index", "nxt"));
    specs.push_back(Text("action", "act"));
    specs.push_back(Text("wave_index", "wav"));
    specs.push_back(Text("prev_index", "prv"));
    specs.push_back(Text("quote_size", "qsz"));
    specs.push_back(Text("quote_type", "qtyp"));
    specs.push_back(Text("seq_index", "seq"));

    specs.push_back(Text("id", "id", normalizeHexToDec));
    specs.push_back(Text("order_id_by_algo", "idalgo"));
    specs.push_back(Text("original", "ori"));
    specs.push_back(Text("parent", "prnt"));
    specs.push_back(Text("min_fill_size", "mqty"));

    // This key contains a space. Enable space-style parsing for this field only.
    specs.push_back(Text("order_index", "order index", normalizeTrim));
    specs.push_back(Text("last_filled_price", "lastPx"));

    // FIX-only fields
    specs.push_back(Tag("sending_time_utc", "60"));
    specs.push_back(Tag("causal_msg_seqnum", "34"));
    specs.push_back(Tag("tif", "59"));

    // DUAL fields
    specs.push_back(Dual("side", "sd", "54"));
    specs.push_back(Dual("price", "px", "44"));
    specs.push_back(Dual("filled_size", "fll", "14"));

    specs.push_back(Dual("exec_inst", "t18", "18", normalizeTrim, normalizeCharToAsciiIfSingle));
    specs.push_back(Dual("broker_id", "brkr", "76"));
    specs.push_back(Dual("target_size", "sz", "38"));

    // COMPUTED
    specs.push_back(Computed("liquidity", &computeLiquidity));
    specs.push_back(Computed("visibility", &computeVisibility));
    specs.push_back(Computed("sending_time_est", &computeSendingTimeEst));
...
static bool writeEnrichedCsv(const std::string& outCsv,
    ...
    for (size_t i = 0; i < originalHeaders.size(); ++i) {
        ...
        std::unordered_map<std::string, std::string>::const_iterator it = kRenameOriginalColumns.find(h);
        std::string outName = (it == kRenameOriginalColumns.end()) ? h : it->second;
        keepIdx.push_back((int)i);
        keepNames.push_back(outName);
    }

    for (size_t i = 0; i < keepNames.size(); ++i) {
        if (i) out << ",";
        out << csvEscape(keepNames[i]);
    }
    ...
    for (size_t r = 0; r < targets.size(); ++r) {
        ...
        for (size_t j = 0; j < keepIdx.size(); ++j) {
            if (j) out << ",";
            int c = keepIdx[j];
            std::string v = (c >= 0 && (size_t)c < t.row.cols.size()) ? t.row.cols[(size_t)c] : "";
