...
static bool needsParentDirectoryRescan(const TargetState& t) {
    ...
    static const char* kParentFields[] = {
        "algo_id", "onleave_internal", "onleave_market", "algo_sender_compid"
    };

    for (size_t i = 0; i < sizeof(kParentFields) / sizeof(kParentFields[0]); ++i) {
        std::unordered_map<std::string, std::string>::const_iterator it = t.outValues.find(kParentFields[i]);
        std::string v = (it == t.outValues.end()) ? "" : trim(it->second);
        if (v.empty() || v == unknownValue()) return true;
    }
    return false;
}
...
static void scanSiblingFilesForUnresolvedParents(const std::string& dirPath,
    ...
    if (!dir) {
        LOG(WARNING) << "Cannot open sibling directory for parent order_id_by_algo scan: " << dirPath;
        return;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        else if (dirPath == ".") fullPath = filename;
        else fullPath = dirPath + "/" + filename;
        ...
        size_t matchedNow = scanSingleFileForTargets(fullPath, stillUnresolved, targets, specs, errors);
        totalMatched += matchedNow;
    }

    closedir(dir);
    LOG(INFO) << "Matched in sibling directory scan (order_id_by_algo fallback): " << totalMatched
              << " dir=" << dirPath;
}
