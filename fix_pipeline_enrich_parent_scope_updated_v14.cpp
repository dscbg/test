...
static void applyDualField(const ExtractorSpec& spec,
    ...
    if (!chosen.empty() && existing != chosen) {
        if (overwriteExisting) {
            target.outValues[spec.out_final_col] = chosen;
            return;
        }
...
static inline bool isMissingOutputValue(const std::unordered_map<std::string, std::string>& outValues,
                                        const std::string& key) {
    std::unordered_map<std::string, std::string>::const_iterator it = outValues.find(key);
    if (it == outValues.end()) return true;
    std::string v = trim(it->second);
    return v.empty() || v == "UNKNOWN";
}
...
static bool needsParentDirectoryRescan(const TargetState& t) {
    ...
    return isMissingOutputValue(t.outValues, "algo_id") ||
           isMissingOutputValue(t.outValues, "onleave_internal") ||
           isMissingOutputValue(t.outValues, "onleave_market") ||
           isMissingOutputValue(t.outValues, "algo_sender_compid");
}
...
static bool needsParentFixRescan(const TargetState& t) {
    if (t.row.orderIdByAlgo.empty()) return false;
    return isMissingOutputValue(t.outValues, "algo_sender_compid");
}
...
static void buildParentOnlyIndex(const std::vector<int>& idxs,
                                 const std::vector<TargetState>& targets,
                                 std::unordered_map<std::string, std::vector<int> >& byParent) {
    byParent.clear();
    for (size_t i = 0; i < idxs.size(); ++i) {
...
static void scanSingleFileForUnresolvedParentsOnly(const std::string& filePath,
    ...
    std::unordered_map<std::string, std::vector<int> > localByParent;
    buildParentOnlyIndex(targetIdxs, targets, localByParent);
