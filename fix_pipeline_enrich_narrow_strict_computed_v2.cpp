...
// NOTE: this does NOT handle daylight saving time; it matches the requirement "UTC -> EST".
...
struct ExtractorSpec {
    ...
    // Optional per-source normalization / casting.
    // - TEXT_KV uses normalize_text
    // - FIX_TAG uses normalize_tag
    // - DUAL_TEXT_TAG uses both, then compares normalized values
...
// SendingTime (tag 60): output EST time string converted from UTC.
// This uses a fixed -5h shift (no DST), per current requirement.
static std::string computeSendingTimeEst(const std::unordered_map<std::string, std::string>& /*textKv*/,
    ...
    if (t60.empty()) return "UNKNOWN";
    std::string est = utcToEstFixed5(trim(t60));
    return est.empty() ? "UNKNOWN" : est;
}
...
static std::vector<ExtractorSpec> buildExtractorSpecs() {
    ...
    specs.push_back({"next_index",   TEXT_KV, "nxt",  "", "", "", "next_index",   "", normalizeTrim, normalizeTrim, NULL});
    specs.push_back({"action",       TEXT_KV, "act",  "", "", "", "action",       "", normalizeTrim, normalizeTrim, NULL});
    specs.push_back({"wave_index",   TEXT_KV, "wav",  "", "", "", "wave_index",   "", normalizeTrim, normalizeTrim, NULL});
    specs.push_back({"prev_index",   TEXT_KV, "prv",  "", "", "", "prev_index",   "", normalizeTrim, normalizeTrim, NULL});
    specs.push_back({"quote_size",   TEXT_KV, "qsz",  "", "", "", "quote_size",   "", normalizeTrim, normalizeTrim, NULL});
    specs.push_back({"quote_type",   TEXT_KV, "qtyp", "", "", "", "quote_type",   "", normalizeTrim, normalizeTrim, NULL});
    specs.push_back({"seq_index",    TEXT_KV, "seq",  "", "", "", "seq_index",    "", normalizeTrim, normalizeTrim, NULL});
    specs.push_back({"exec_id_xcid", TEXT_KV, "xcid", "", "", "", "exec_id_xcid", "", normalizeTrim, normalizeTrim, NULL});

    // FIX-only fields
    specs.push_back({"sending_time_utc", FIX_TAG, "", "60", "", "", "sending_time_utc", "", normalizeTrim, normalizeTrim, NULL});

    // DUAL-source fields (text key + FIX tag)
    specs.push_back({"broker_id", DUAL_TEXT_TAG, "brkr", "76",
                     "broker_id_text", "broker_id_tag76", "broker_id_final", "broker_id_mismatch", normalizeTrim, normalizeTrim, NULL});

    specs.push_back({"target_size", DUAL_TEXT_TAG, "sz", "38",
                     "target_size_text", "target_size_tag38", "target_size_final", "target_size_mismatch", normalizeTrim, normalizeTrim, NULL});
    ...
    specs.push_back({"liquidity",  COMPUTED, "", "", "", "", "liquidity",  "", NULL, NULL, &computeLiquidity});
    specs.push_back({"visibility", COMPUTED, "", "", "", "", "visibility", "", NULL, NULL, &computeVisibility});
    specs.push_back({"sending_time_est", COMPUTED, "", "", "", "", "sending_time_est", "", NULL, NULL, &computeSendingTimeEst});
    ...
    for (size_t i = 0; i < specs.size(); ++i) {
        ...
        if (spec.source_type == TEXT_KV) {
            ...
            if (itText != textKv.end()) {
                NormalizeFn nt = spec.normalize_text ? spec.normalize_text : normalizeTrim;
                std::string vText = nt ? nt(itText->second) : itText->second;
            ...
            if (itTag != fixTags.end()) {
                NormalizeFn nt = spec.normalize_tag ? spec.normalize_tag : normalizeTrim;
                std::string vTag = nt ? nt(itTag->second) : itTag->second;
