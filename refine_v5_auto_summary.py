...
def dominant_success_cols(day_df: pd.DataFrame, top_n: int = 2):
    cols = ["ct_success_rate", "spread_success_rate", "tick_success_rate"]
    means = {c: pd.to_numeric(day_df[c], errors="coerce").mean() for c in cols if c in day_df.columns}
    ranked = sorted(means.items(), key=lambda kv: kv[1])
    return [k for k, _ in ranked[:top_n]]


def pretty_success_name(col: str) -> str:
    mapping = {
        "ct_success_rate": "ct success",
        "spread_success_rate": "spread success",
        "tick_success_rate": "tick success",
    }
    return mapping.get(col, col)


def classify_region(row: pd.Series, coverage_low: float, coverage_high: float, eligible_ticker_passrate: float) -> str:
    cov = float(row["coverage_mean"])
    tkr = float(row["ticker_ratio_mean_by_day"])
    spread_s = float(row["spread_success_rate"])
    ct_s = float(row["ct_success_rate"])
    tick_s = float(row["tick_success_rate"])

    if cov > coverage_high + 0.05 and tkr > max(0.25, eligible_ticker_passrate * 2.5):
        status = "too loose"
    elif cov < max(0.05, coverage_low - 0.03):
        status = "too strict"
    else:
        status = "reasonable"

    succ = {"ct": ct_s, "spread": spread_s, "tick": tick_s}
    driver = min(succ, key=succ.get)
    return f"{status}; dominant constraint = {driver}"
...
def make_region_dashboard(region, best_row, day_eval, ticker_ratio_eval,
    ...
    # Panel 1: coverage + top 2 lowest success rates together
    d1 = day_eval.copy()
    ...
    for col in dominant_success_cols(d1, top_n=2):
        axes[0].plot(d1["date"], d1[col], label=pretty_success_name(col))
    axes[0].axhline(coverage_low, linestyle="--")
    axes[0].axhline(coverage_high, linestyle="--")
    axes[0].set_title(f"{region}: coverage + key success rates\n{combo_label}")
    axes[0].legend()

    # Panel 2: ticker ratio
    ...
    axes[1].plot(tr["date"], tr["pct_tickers_ge_thr"], label="daily")
    axes[1].plot(tr["date"], tr["roll5"], label="roll5")
    axes[1].axhline(eligible_ticker_passrate, linestyle="--")
    axes[1].set_title(f"{region}: ticker ratio over time")
    axes[1].legend()
...
def write_text_summary(best_df: pd.DataFrame, coverage_low: float, coverage_high: float,
                       eligible_ticker_passrate: float, out_dir: Path):
    best_df = best_df.copy()
    best_df["auto_assessment"] = best_df.apply(
        lambda r: classify_region(r, coverage_low, coverage_high, eligible_ticker_passrate), axis=1
    )

    best_df[[
        "region", "X_min", "Y_bps", "Z_ticks", "coverage_mean", "ticker_ratio_mean_by_day",
        "n_symbols_eligible", "ct_success_rate", "spread_success_rate", "tick_success_rate",
        "auto_assessment"
    ]].to_csv(out_dir / "auto_region_conclusions.csv", index=False)
    ...
    for _, r in best_df.iterrows():
        lines.append(
            f"## {r['region']}\n"
            f"- Best combo: X={r['X_min']:.2f}, Y={r['Y_bps']:.2f}, Z={r['Z_ticks']:.1f}\n"
            f"- Coverage mean: {r['coverage_mean']:.3f}\n"
            f"- Daily eligible ticker ratio mean: {r['ticker_ratio_mean_by_day']:.3f}\n"
            f"- Eligible tickers: {int(r['n_symbols_eligible'])}\n"
            f"- Success rates: ct={r['ct_success_rate']:.3f}, spread={r['spread_success_rate']:.3f}, tick={r['tick_success_rate']:.3f}\n"
            f"- Conclusion: {r['auto_assessment']}\n"
        )
...
def main():
    ...
    write_text_summary(best, args.coverage_target_low, args.coverage_target_high,
                       args.eligible_ticker_passrate, out_dir)
    ...
    manifest = pd.DataFrame([
        ...
        {"file": "plots/<REGION>.png", "description": "Best-only 2-panel dashboard per region"},
    ])
