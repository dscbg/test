...
enum SourceType { TEXT_KV, FIX_TAG, DUAL_TEXT_TAG };
...
static std::vector<ExtractorSpec> buildExtractorSpecs() {
    ...
    specs.push_back({"next_index", TEXT_KV, "nxt", "", "", "", "next_index", "", normalizeTrim});
    specs.push_back({"action",     TEXT_KV, "act", "", "", "", "action",     "", normalizeTrim});
    specs.push_back({"wave_index", TEXT_KV, "wav", "", "", "", "wave_index", "", normalizeTrim});
    specs.push_back({"prev_index", TEXT_KV, "prv", "", "", "", "prev_index", "", normalizeTrim});
    specs.push_back({"quote_size", TEXT_KV, "qsz", "", "", "", "quote_size", "", normalizeTrim});
    specs.push_back({"quote_type", TEXT_KV, "qtyp","", "", "", "quote_type", "", normalizeTrim});
    specs.push_back({"seq_index",  TEXT_KV, "seq", "", "", "", "seq_index",  "", normalizeTrim});
    specs.push_back({"exec_id_xcid",TEXT_KV, "xcid","", "", "", "exec_id_xcid","", normalizeTrim});

    // DUAL-source fields (text key + FIX tag)
    specs.push_back({"broker_id", DUAL_TEXT_TAG, "brkr", "76",
                     "broker_id_text", "broker_id_tag76", "broker_id_final", "broker_id_mismatch", normalizeTrim});

    specs.push_back({"target_size", DUAL_TEXT_TAG, "sz", "38",
                     "target_size_text", "target_size_tag38", "target_size_final", "target_size_mismatch", normalizeTrim});
...
static std::vector<std::string> buildEnrichedColumns(const std::vector<ExtractorSpec>& specs) {
    ...
    for (size_t i = 0; i < specs.size(); ++i) {
        const ExtractorSpec& spec = specs[i];
        if (spec.source_type == DUAL_TEXT_TAG) {
            cols.push_back(spec.out_text_col);
            cols.push_back(spec.out_tag_col);
