...
static inline std::string extractParentTextIdRaw(const std::string& line) {
    return extractTextValueColon(line, "id");
}
...
static bool loadParentMap(const std::string& mapCsv,
    ...
    for (size_t i = 0; i < headers.size(); ++i) {
        ...
        else if (iequals(h, "order_id_by_algo") || iequals(h, "OrderIDByAlgo") ||
                 iequals(h, "parent") || iequals(h, "parent_id") || iequals(h, "ParentID")) pIdx = (int)i;
    }
    if (clIdx < 0 || pIdx < 0) {
        LOG(ERROR) << "Parent map CSV must contain ClOrdID and order_id_by_algo (or parent_id) columns.";
        return false;
    }
...
static size_t scanSingleFileForTargets(const std::string& filePath,
    ...
    while (std::getline(in, line)) {
        ...
        bool isParentTextLine = !textRawParentId.empty() && hasParentTextMarker(line);
...
static void scanLogDirectorySubset(const std::string& logDir,
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        while (std::getline(in, line)) {
            ...
            std::string textXcid  = textKv["xcid"];
            bool isParentTextLine = !textRawId.empty() && hasParentTextMarker(line);
...
static bool writeParentMissingDebugCsv(const std::string& outCsv,
    ...
    for (size_t i = 0; i < targets.size(); ++i) {
        ...
        std::string orderId = t.row.parentId;
